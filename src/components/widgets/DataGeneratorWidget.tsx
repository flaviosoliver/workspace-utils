...
import { Copy, RefreshCw, Download, User, Mail, Phone, MapPin, Calendar, Building, CreditCard } from 'lucide-react';
...
export default function DataGeneratorWidget() {
  ...
  const firstNames = ['Ana', 'João', 'Maria', 'Pedro', 'Carla', 'Lucas', 'Fernanda', 'Rafael', 'Juliana', 'Bruno', 'Camila', 'Diego', 'Larissa', 'Thiago', 'Beatriz'];
  ...
  const domains = ['gmail.com', 'hotmail.com', 'yahoo.com', 'outlook.com', 'uol.com.br', 'terra.com.br'];
  const cities = ['São Paulo', 'Rio de Janeiro', 'Belo Horizonte', 'Salvador', 'Brasília', 'Fortaleza', 'Curitiba', 'Recife', 'Porto Alegre', 'Manaus'];
  const companies = ['Tech Solutions', 'Inovação Digital', 'Sistemas Avançados', 'Consultoria Plus', 'Desenvolvimento Web', 'Software House', 'TI Moderna', 'Código Limpo'];
  ...
  const generatePerson = () => {
    ...
    return {
      ...
      birthDate: generateRandomDate(new Date(1940, 0, 1), new Date(2005, 11, 31)).toLocaleDateString('pt-BR'),
  ...
  const generateAddress = () => {
    const streets = ['Rua das Flores', 'Avenida Principal', 'Rua do Comércio', 'Avenida Central', 'Rua da Paz', 'Avenida Brasil'];
    const neighborhoods = ['Centro', 'Jardim América', 'Vila Nova', 'Bela Vista', 'Copacabana', 'Ipanema'];
    ...
    return {
      ...
      complement: Math.random() > 0.5 ? `Apto ${generateRandomNumber(1, 200)}` : ''
    };
  };
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-white capitalize">
              {type === 'creditCard' ? 'Cartão de Crédito' : 
               type === 'person' ? 'Pessoa' :
               type === 'company' ? 'Empresa' : 'Endereço'}
            </span>
          </div>
          <button
      ...
      <div className="flex-1 overflow-y-auto">
        {generatedData.length === 0 ? (
          <div className="text-center py-8 text-gray-400">
            ...
            <p className="text-sm">Selecione o tipo e clique em "Gerar Dados"</p>
          </div>
        ) : (
